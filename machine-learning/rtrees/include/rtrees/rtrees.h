...
class RTrees {
public:
	RTrees(int k, int numberOfComponents, DataExtractionMethod method)
		: m_numberOfComponents(numberOfComponents),
		m_method(method)
	{
		m_rtreesPtr = cv::ml::RTrees::create();
		m_preprocessor = std::make_shared<Preprocessing>(-1, m_numberOfComponents, m_method);
	}
...
private:
	...
	cv::Ptr<cv::ml::RTrees> m_rtreesPtr;
